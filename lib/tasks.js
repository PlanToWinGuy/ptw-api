...
import { safeUpsertEventForTask } from './googleCalendar.js';
...
export async function completeTask(sql, user, taskId, completionPercentage = 100, actualMinutesSpent = null) {
  ...
  // Best-effort Google Calendar cleanup/push (see lib/googleCalendar.js) -- a completed
  // task's event (if any) gets deleted since it's no longer a real Pending commitment;
  // a partial-completion remainder task is a brand-new Pending, timed task, so it gets
  // pushed as its own event. Both are no-ops when sync isn't connected/enabled.
  await safeUpsertEventForTask(sql, user, updated[0]);
  if (remainderTask) await safeUpsertEventForTask(sql, user, remainderTask);
