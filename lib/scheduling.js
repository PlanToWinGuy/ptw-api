...
const REFLECT_PATTERN = /\bjournal\b|\breflect(ion)?\b|\bwrite (a |one-paragraph )?(reflection|down)\b|\btake a note (on|about)\b|\bgratitude\b|\bwrite (one |a )?(sentence|line|note)s? (about|on)\b|\blog (one|a) (line|note|sentence)s? (per|about|on)\b/i;
