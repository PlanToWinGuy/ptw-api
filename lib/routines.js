import { sql, pillarIdFromName } from './db.js';
import { addMinutesToClock, findOpenSlot } from './scheduling.js';

const WEEKDAY_NAMES = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

// Real, mandatory bookend routines created once at profile-creation time, since the
// onboarding questionnaire already collects wake-up/wind-down times but nothing was ever
// built to actually create anything from them (users.wake_time/wind_down_time existed in
// the schema completely unused until now). Category 'General' -- not a real pillar name,
// so materializeRoutinesForDate resolves pillar_id to null -- these are universal, not
// tied to whichever pillar happens to be active. Idempotent: skips a routine that already
// exists by name, so this is safe to call more than once for the same user.
export async function createBookendRoutines(userId, wakeTime, windDownTime) {
  const existing = await sql`SELECT name FROM routines WHERE user_id = ${userId} AND name IN ('Morning Routine', 'Wind-Down Routine')`;
  const existingNames = new Set(existing.map(r => r.name));

  if (wakeTime && !existingNames.has('Morning Routine')) {
    const steps = [
      { name: 'Brush teeth', durationMinutes: 3 },
      { name: 'Wash face', durationMinutes: 2 },
      { name: 'Shower', durationMinutes: 10 },
      { name: '5-minute stretch', durationMinutes: 5 },
    ];
    await sql`
      INSERT INTO routines (user_id, name, icon, category, is_active, schedule_days, schedule_time, steps)
      VALUES (${userId}, 'Morning Routine', '☀️', 'General', true, ${[]}, ${wakeTime}::time, ${JSON.stringify(steps)}::jsonb)
    `;
  }

  if (windDownTime && !existingNames.has('Wind-Down Routine')) {
    const steps = [
      { name: 'Shower', durationMinutes: 10 },
      { name: 'Brush teeth', durationMinutes: 3 },
      { name: 'Reflect on your day', durationMinutes: 5 },
    ];
    const totalMin = steps.reduce((s, st) => s + st.durationMinutes, 0);
    const startTime = addMinutesToClock(windDownTime, -totalMin); // ends AT wind-down time, not starts at it
    await sql`
      INSERT INTO routines (user_id, name, icon, category, is_active, schedule_days, schedule_time, steps)
      VALUES (${userId}, 'Wind-Down Routine', '🌙', 'General', true, ${[]}, ${startTime}::time, ${JSON.stringify(steps)}::jsonb)
    `;
  }
}

// Lazy materialization: rather than a real nightly cron (new infra this project
// doesn't have), a routine gets turned into a real Logging Task for a given date the
// first time anything asks for that date -- idempotent (checks routine_id+due_date
// first), so it's safe to call this on every /api/user-projects request.
export async function materializeRoutinesForDate(user, targetDate) {
  const weekday = WEEKDAY_NAMES[new Date(targetDate + 'T00:00:00').getDay()];
  // end_date IS NULL means indefinite (every user-created routine, and habit/mindset
  // goals' daily_anchor) -- a project-linked recurring action stops materializing once
  // targetDate passes it. Compared in SQL, not JS, since a DATE column comes back from
  // the driver as a native Date object rather than a plain string (see Pass 1's fix in
  // api/tasks/update-completion.js for the same footgun).
  const routines = await sql`
    SELECT * FROM routines
    WHERE user_id = ${user.id} AND is_active = true
      AND (end_date IS NULL OR end_date >= ${targetDate})
  `;

  for (const r of routines) {
    const days = r.schedule_days || [];
    if (days.length && !days.includes(weekday)) continue;

    const existing = await sql`SELECT id FROM tasks WHERE routine_id = ${r.id} AND due_date = ${targetDate}`;
    if (existing.length) continue;

    const steps = r.steps || [];
    const totalMin = steps.reduce((s, st) => s + (Number(st.durationMinutes) || 0), 0) || 15;
    const pillarId = pillarIdFromName(r.category);

    // r.schedule_time is this routine's own preferred slot, but several routines/goals
    // across different pillars can default to the same time-of-day and land on top of
    // each other (confirmed live: two pillars' "Morning" routines both materializing at
    // the exact same clock time). findOpenSlot both checks whether that slot's actually
    // free (against other tasks and fixed commitments already on this date) and finds
    // the next real gap if not -- searching forward from the routine's own preferred
    // time rather than the default 08:00 day-start, so it still lands as close to its
    // intended time as possible instead of every conflicting routine collapsing onto
    // whatever the first free slot of the day happens to be.
    const preferredClock = r.schedule_time ? String(r.schedule_time).slice(0, 5) : '08:00';
    const slot = await findOpenSlot(sql, user.id, {
      earliestDate: targetDate, searchDays: 1, durationMinutes: totalMin, dayStartClock: preferredClock,
    });

    await sql`
      INSERT INTO tasks (user_id, goal_id, pillar_id, routine_id, name, kind, estimated_duration_minutes, due_date, start_time, priority, tool_hint)
      VALUES (${user.id}, ${r.goal_id || null}, ${pillarId}, ${r.id}, ${r.name}, 'habit', ${totalMin}, ${slot.date}, ${slot.startTime}, 'Medium', ${r.tool_hint || null})
    `;
  }
}

// Opt-in "drink water" nudges (Diet Preferences' Water Reminders field) -- same real
// routine/materialization mechanism as api/goals.js's breakfast/lunch/dinner log
// reminders, just spread across several times a day instead of one fixed meal time.
// Called from api/preferences/[scope].js whenever the diet scope is saved with a
// waterReminders value.
const WATER_REMINDER_INTERVAL_HOURS = { 'Every 2 hours': 2, 'Every 3 hours': 3, 'Every 4 hours': 4 };
const MAX_WATER_REMINDERS_PER_DAY = 8; // safety cap -- a tiny interval + a long waking window shouldn't spam the schedule

export async function syncWaterReminderRoutines(userId, wakeTime, windDownTime, waterRemindersPref) {
  // Rebuilt from scratch every save -- idempotent no matter how many times preferences
  // are saved, and cleanly clears all reminders the moment the user picks 'Off'. tool_hint
  // 'water' (not name) is the sync key, so a routine renamed elsewhere never leaks in.
  await sql`DELETE FROM routines WHERE user_id = ${userId} AND tool_hint = 'water'`;

  const hours = WATER_REMINDER_INTERVAL_HOURS[waterRemindersPref];
  if (!hours) return; // 'Off' or unrecognized -- no reminders wanted

  const start = String(wakeTime || '08:00').slice(0, 5);
  const end = String(windDownTime || '21:00').slice(0, 5);
  const [sh, sm] = start.split(':').map(Number);
  const [eh, em] = end.split(':').map(Number);
  let startMin = sh * 60 + sm;
  let endMin = eh * 60 + em;
  if (endMin <= startMin) endMin += 24 * 60; // wind-down past midnight

  const steps = [{ name: 'Drink Water', durationMinutes: 2 }];
  // First nudge half an interval after waking (not AT wake time itself, which is already
  // claimed by the Morning Routine bookend), then every `hours` after that.
  let t = startMin + Math.round((hours * 60) / 2);
  let count = 0;
  while (t < endMin && count < MAX_WATER_REMINDERS_PER_DAY) {
    const hh = String(Math.floor((t / 60) % 24)).padStart(2, '0');
    const mm = String(t % 60).padStart(2, '0');
    await sql`
      INSERT INTO routines (user_id, name, icon, category, is_active, schedule_days, schedule_time, steps, tool_hint)
      VALUES (${userId}, 'Drink Water', '💧', 'Diet', true, ${[]}, ${hh + ':' + mm}::time, ${JSON.stringify(steps)}::jsonb, 'water')
    `;
    t += hours * 60;
    count++;
  }
}
